...
<head>
    ...
    <link href="./newaccount_create.css" rel="stylesheet">
...
<body>
    ...
    <main class="main">
        ...
        <div class="account">
            <form  id="accountform" action=""　data-persist="garlic"> <!-- 完了画面に遷移 -->
                <dl class="form-area">
                    <dt><span class="required">氏名(全角)</span></dt>
                    <dd><input id="name" class="input-text validate[required,maxSize[16]]" type="text" name="name" placeholder="山田太郎"></dd> <!--name属性はフォームを受信したプログラムが各項目を判別するための属性、requiredは必須項目なので入力がなければ警告を表示-->
                    <!-- <dd id="name_chk" style="display: none; color: red;">名前が漢字になっていません</dd> -->

                    <dt><span class="required">氏名(ふりがな)</span></dt>
                    <dd><input class="input-text validate[required,maxSize[16]]" type="text" name="name_huri" placeholder="やまだたろう" id="name_huri"></dd>
                    <!-- <dd id="name_huri_chk" style="display: none; color: red;">名前(ふりがな)がひらがなになっていません</dd> -->

                    <dt><span class="required">ユーザネーム</span></dt>
                    <dd><input class="input-text validate[required,maxSize[256]]" type="text" name="username" id="username"></dd>
                    <!-- <dd id="mail_chk" style="display: none; color: red;">メールアドレスが形式に沿っていません</dd> -->

                    <dt><span class="required">性別</span></dt>
                    <dd>
                        <input type="radio" id="man" class="validate[required]" name="sex" value="man" checked />
                        <label for="a">男性</label>
                        <input type="radio" id="woman" class="validate[required]" name="sex" value="woman" />
                        <label for="b">女性</label>
                        <input type="radio" id="notsex" class="validate[required]" name="sex" value="nosex" />
                        <label for="o">回答しない</label>
                    </dd>

                    <dt><span for="birth" class="required">生年月日</span></dt>
                    <dd>
                        <select id="year" class="validate[required]"><option value="0">----</option></select> 年
                        <select id="month" class="validate[required]"><option value="0">--</option></select> 月
                        <select id="date" class="validate[required]"><option value="0">--</option></select> 日
                    </dd>

                    <dt><span class="">郵便番号</span></dt>
                    <dd><input class="input-text validate[custom[number],maxSize[7]]" type="text" name="yuubin" placeholder="ハイフンなし" id="yuubin"></dd>
                    <!-- <dd id="yuubin_chk" style="display: none; color: red;">郵便番号が7桁になっていません</dd> -->
                    ...
                    <dd><select name="pref" id="pref" class="input-text validate[required]">
                    ...
                    <dd><input class="input-text validate[required,maxSize[16]]" type="text" name="address_shi" placeholder="市区町村のみ入力" id="address_shi"></dd>

                    <dt><span class="required">電話番号</span></dt>
                    <dd><input class="input-text validate[required,custom[phone],maxSize[21]]" type="tel" name="tel" pattern="[\d-]*" placeholder="ハイフンなし" id="tel"></dd>
                    <!-- <dd id="phone_chk" style="display: none; color: red;">電話番号が形式に沿っていません</dd> -->

                    <dt><span class="required">メールアドレス</span></dt>
                    <dd><input class="input-text validate[required,custom[email],maxSize[256]]" type="text" name="email" id="mail"></dd>
                    <!-- <dd id="mail_chk" style="display: none; color: red;">メールアドレスが形式に沿っていません</dd> -->

                    <dt><span class="required">パスワード</span></dt>
                    <dd><input class="input-text validate[required,minSize[8],maxSize[16]]" type="password" name="password" placeholder="半角英数8文字以上" id="pass"></dd>
                    <!-- <dd id="pass_chk" style="display: none; color: red;">パスワードが半角英数8文字以上になっていません</dd> -->

                    <dt><span class="required">パスワード(確認)</span></dt>
                    <dd><input class="input-text validate[required,equals[pass],minSize[8],maxSize[16]]" type="password" name="password" placeholder="半角英数8文字以上" id="repass"></dd>
